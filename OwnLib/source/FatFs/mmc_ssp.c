...
#define PCLK_SSP	120000000UL	/* PCLK frequency to be supplied for SSP [Hz] */
...
DEFINE_PIN(MMC_CD, 2, 11)
/* Card detect (yes:true, no:false, default:true) */
DEFINE_PIN(CS, 1, 19)
...
#define FCLK_FAST() (SSPxCR0 |= (((SystemCoreClock/(2*SCLK_FAST))-1)<<8))
#define FCLK_SLOW() (SSPxCR0 |= (((SystemCoreClock/(2*SCLK_SLOW))-1)<<8))
...
UINT btr /* Number of bytes to receive (16, 64 or 512) */
	...
	while (btr >= 2) { /* Receive the data block into buffer */
		btr -= 2;
		while (!(SSPxSR & _BIT(2)))
			; /* Wait for any data in receive FIFO */
		d = SSPxDR;
		SSPxDR = 0xFFFF;
		*buff++ = d >> 8;
		*buff++ = d;
...
UINT btx /* Number of bytes to send (multiple of 16) */
	...
	while (btx >= 2) { /* Transmit data block */
		...
		d = *buff++;
		d = d << 8 | *buff++;
		while (!(SSPxSR & _BIT(2)))	; /* Wait for any data in receive FIFO */
		SSPxDR;
		SSPxDR = d;
	}
...
DWORD arg /* Argument */
	...
	if (cmd == CMD12)
		xchg_spi(0xFF); /* Discard following one byte when CMD12 */
...
DSTATUS disk_initialize(BYTE drv /* Physical drive number (0) */
	...
	if (Stat & STA_NODISK)
		return Stat; /* Is a card existing in the soket? */
	...
	if (send_cmd(CMD0, 0) == 1) { /* Put the card SPI state */
		...
		if (send_cmd(CMD8, 0x1AA) == 1) { /* Is the catd SDv2? */
			...
			while (Timer1 && send_cmd(cmd, 0))
				; /* Wait for the card leaves idle state */
...
void disk_timerproc(void) {
	...
	if (!MMC_CD_READ()) { /* Card is in socket */
		s &= ~STA_NODISK;
	} else { /* Socket empty */
