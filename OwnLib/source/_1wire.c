...
{
	Chip_GPIO_SetDir( LPC_GPIO , OW_PORT , OW_PIN_MASK , direction );
}
...
static void OW_WRITE_PIN(LOGIC_LEVEL_eType bitstate)
{
	switch (bitstate){
		case LOW:
			Chip_GPIO_SetPortOutLow( LPC_GPIO , OW_PORT , OW_PIN );
			break;
		case HIGH:
			Chip_GPIO_SetPortOutHigh(LPC_GPIO , OW_PORT , OW_PIN );
			break;
		default:
			break;
	}
...
{
	return Chip_GPIO_ReadPortBit(LPC_GPIO , OW_PORT , OW_PIN);
}
...
PRESENCE_t OW_reset_pulse(void)
{
	...
	_delay_uS(DELAY_55Us);		// delay 70 microsecond (us)
	presence = read_OW();			//Sample for presence pulse from slave
	_delay_uS(DELAY_480Us);	  			// delay 200 microsecond (us)
	drive_OW_high ();		    	// Release the bus
