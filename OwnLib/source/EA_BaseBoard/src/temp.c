...
#include <EA_BaseBoard/inc/temp.h>
...
#ifdef TEMP_USE_P0_6
#define P0_6_STATE ((Chip_GPIO_ReadValue(LPC_GPIO,0) & (1 << 6)) != 0)
#define GET_TEMP_STATE P0_6_STATE
#else
#define P0_2_STATE ((Chip_GPIO_ReadValue(LPC_GPIO,0) & (1 << 2)) != 0)
...
{
#ifdef TEMP_USE_P0_6
    Chip_GPIO_SetDir(LPC_GPIO, 0, (1<<6), 0 );
#else
    Chip_GPIO_SetDir(LPC_GPIO, 0, (1<<2), 0 );
#endif
    getTicks = getMsTicks;
}
