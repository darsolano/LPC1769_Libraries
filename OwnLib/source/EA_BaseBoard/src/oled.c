...
#include <string.h>
#include "EA_BaseBoard/inc/oled.h"
...
//static const uint8_t OLED_Setup_Codes[] = {
//		    OLED_DISP_OFF,
//			OLED_SET_CONTRAST,
//			0xCF,
//
...
static int I2CWrite(uint8_t addr, uint8_t* buf, uint32_t len)
{
	...
	if (Chip_I2C_MasterTransfer(I2CDEV, &txsetup) == SUCCESS)
	{
		return (0);
	}
	else
...
static void runInitSequence(void)
{
	 ...
	 */
//	int i;
//	int t = sizeof(OLED_Setup_Codes)/sizeof(uint8_t);
//
//	for (i=0;i<t;i++){
//		writeCommand(OLED_Setup_Codes[i]);
//	}
...
#ifdef OLED_USE_I2C
	/* Init I2C */
	Board_I2C_Init(I2CDEV);
	Chip_I2C_Init(I2CDEV);
	Chip_I2C_SetClockRate(I2CDEV , 400000);
...
#else
	Board_SSP_Init(LPC_SSP1, 5000000);
	OLED_CS_OFF();
#endif
