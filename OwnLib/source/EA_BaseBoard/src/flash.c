...
#include <EA_BaseBoard/inc/flash.h>
...
#define FLASHSSP			LPC_SSP1

DEFINE_PIN(FLASH_CS , 1 ,18)	//CS substitute PIN for board pin
...
static void readDeviceId(uint8_t *deviceId)
{
    ...
    SPIxchg( FLASH_CMD_RDID );
    for (i=0;i<4;i++){
    	deviceId[i] = SPIxchg( 0 );
    }
    FLASH_CS_DESELECTED();
}
...
static uint8_t readStatus(void)
{
    ...
    //SSPSend( FLASH_CMD_RDSR );
    status = SPIxchg( FLASH_CMD_RDSR );
...
//    Board_SSP_Init(FLASHSSP,20000000);
//    Chip_SSP_Init(FLASHSSP);
//    Chip_SSP_Enable(FLASHSSP);
//    Chip_SSP_SetBitRate(FLASHSSP, 1000000);


    FLASH_CS_OUTPUT();	// As Output 1.18 baseboard
    FLASH_CS_DESELECTED();
