...
#include <EA_BaseBoard/inc/uart2.h>
...
static int I2CRead(uint8_t addr, uint8_t* buf, uint32_t len)
{
	...
	rxsetup.slaveAddr = addr;
	rxsetup.txBuff = NULL;	// Get address to read at writing address
	rxsetup.txSz = 0;
	rxsetup.rxBuff = buf;
	rxsetup.rxSz = len;

	if (Chip_I2C_MasterTransfer(I2C2, &rxsetup) == SUCCESS){
		return (0);
	} else {
		return (-1);
	}
...
static int I2CWrite(uint8_t addr, uint8_t* buf, uint32_t len)
{
	...
	if (Chip_I2C_MasterTransfer(I2C2, &txsetup) == SUCCESS){
		return (0);
	} else {
		return (-1);
	}
...
void uart2_init (uint32_t baudRate, uart2_channel_t chan)
{
    ...
    Chip_GPIO_SetDir(LPC_GPIO,0, 1<<9, 1);
    Chip_GPIO_SetDir(LPC_GPIO,2, 1<<8, 1);

    Chip_GPIO_SetDir(LPC_GPIO,0, 1<<9, 1); // SI-A1
    Chip_GPIO_SetDir(LPC_GPIO,2, 1<<8, 1); // CS#-A0
