...
static void setLeds(void)
{
    ...
    buf[3] = ls[2];
    buf[4] = ls[3];
    Chip_I2C_MasterSend(I2C2, PCA9532_I2C_ADDR, buf, 5);
}
...
uint16_t pca9532_getLedState (uint32_t shadow)
{
    ...
    if (shadow) {
         ...
         */
        Chip_I2C_MasterCmdRead(I2C2, PCA9532_I2C_ADDR, PCA9532_INPUT0, buf, 2);
        ret = buf[0];
        ret |= (buf[1] << 8);
...
void pca9532_setBlink0Period(uint8_t period)
{
    ...
    buf[0] = PCA9532_PSC0;
    buf[1] = period;
    Chip_I2C_MasterSend(I2C2, PCA9532_I2C_ADDR, buf, 2);
}
