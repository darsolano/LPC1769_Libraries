 ...
 * PINS CS		0.16
 */
#define tft32Xmit				LPC_SSP0
DEFINE_PIN(shift_CS, 0,16)
 ...
 */
void tft32_SpiIO(uint8_t TX_Data) {
	while (!(tft32Xmit->SR & SSP_STAT_TFE));
	tft32Xmit->DR = TX_Data;	// send data
//	while (!(tft32Xmit->SR & SSP_STAT_RNE));
//	return tft32Xmit->DR;	// Receive data
}
#endif
...
void tft32_serial3wire_write_word(uint32_t data)
{
	uint32_t temp;
	int count;
...
void tft32_PINS(void) {
	...
	Chip_SSP_Init(tft32Xmit);
	Chip_SSP_SetBitRate(tft32Xmit, 5000000);
	Chip_SSP_SetMaster(tft32Xmit, ENABLE);
	Chip_SSP_Enable(tft32Xmit);
	shift_CS_OUTPUT();
	shift_CS_DESELECTED();
#endif
}
...
#if BITBANG
	// For CMD TFT32_LCD_RS must be Zero
	// Set up initial state from inactive
	control = 0;
	control = (TFT32_LCD_LED | TFT32_LCD_RST) & ~(TFT32_LCD_WR | TFT32_LCD_RS | TFT32_LCD_CS);
	tft32_data_out = (cmd | (control<<16)) & 0x001fffff;
	//Start of SPI Xfer
	tft32_serial3wire_write_word(tft32_data_out);

	// Select active active for data transfer
	control |=  (TFT32_LCD_CS | TFT32_LCD_WR);
	tft32_data_out = (cmd | (control<<16)) & 0x001fffff;
	//Start of SPI Xfer
	tft32_serial3wire_write_word(tft32_data_out);
	//End of SPI Xfer
...
#if BITBANG
	// For DATA TFT32_LCD_RS must be One
	// Set up initial state from inactive
	control = 0;
	control = (TFT32_LCD_LED | TFT32_LCD_RST | TFT32_LCD_RS) & ~(TFT32_LCD_WR | TFT32_LCD_CS);
	tft32_data_out = (data | (control<<16)) & 0x001fffff;
	//Start of SPI Xfer
	tft32_serial3wire_write_word(tft32_data_out);
...
void tft32_backlight_on(void){
	tft32_serial3wire_write_word(0x00100000);	// reset hc595
}


void tft32_backlight_off(void){
	tft32_serial3wire_write_word(0x00000000);	// reset hc595
}
#endif
...
/*
	 ...
	 tft32_main_W_com_data(0x03,0xA8A4);
	 tft32_main_W_com_data(0x0C,0x0000);
	 tft32_main_W_com_data(0x0D,0x080C);
	 tft32_main_W_com_data(0x0E,0x2B00);
	 tft32_main_W_com_data(0x1E,0x00B7);
	 tft32_main_W_com_data(0x01,0x2B3F);
	 tft32_main_W_com_data(0x02,0x0600);
	 tft32_main_W_com_data(0x10,0x0000);
	 tft32_main_W_com_data(0x11,0x6070);
	 tft32_main_W_com_data(0x05,0x0000);
	 tft32_main_W_com_data(0x06,0x0000);
	 tft32_main_W_com_data(0x16,0xEF1C);
	 tft32_main_W_com_data(0x17,0x0003);
	 tft32_main_W_com_data(0x07,0x0233);
	 tft32_main_W_com_data(0x0B,0x0000);
...
void tft32_printChar(byte c, int x, int y) {
	...
	if (ptft->orient == PORTRAIT) {
		...
		ptemp =
				(cfont.font + ((c - cfont.offset) * cfont.y_size * cfont.Hrows));
		...
		for (j = 0; j < ((cfont.x_size / 8) * cfont.y_size);
				j += (cfont.x_size / 8)) {
			tft32_setXY(x, y + (j / (cfont.x_size / 8)), x + cfont.x_size - 1,
					y + (j / (cfont.x_size / 8)));
...
void tft32_drawBitmap_(int x, int y, int sx, int sy, uint16_t* data, int scale) {
	...
	if (scale == 1) {
		...
		if (ptft->orient == PORTRAIT) {
			for (ty = 0; ty < sy; ty++) {
				tft32_setXY(x, y + (ty * scale), x + ((sx * scale) - 1),
						y + (ty * scale) + scale);
...
void tft32_drawBitmap(int x, int y, int sx, int sy, uint16_t* data, int deg,
		int rox, int roy) {
	unsigned int col;
	int tx, ty, newx, newy;
	//byte r, g, b;
